...
func FetchSchema(ctx context.Context, client *graphql.Client, endpoint string, headers map[string]string) (*Schema, error) {
	req := graphql.Request{
		Query: IntrospectionQuery,
	}

	result, err := client.Execute(ctx, endpoint, req, headers)
	if err != nil {
